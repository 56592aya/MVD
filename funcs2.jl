...
function optimize_γi!(model::MVD, i::Int64)
	@inbounds for I in eachindex(model.γ[i])
		model.γ[i][I] = model.α[I] + .5 + model._sumπ1_i[I] + model._sumπ2_i[I]
	end
end
function optimize_γd!(model, γd,sumπ1_i,sumπ2_i)
	@inbounds for I in eachindex(γd)
		γd[I] = model._α[I] + .5 + sumπ1_i[I] + sumπ2_i[I]
	end
end
function optimize_γd_perp!(model, γd,sumπ1_i,sumπ2_i)
	@inbounds for I in eachindex(γd)
		γd[I] = model._α[I] + sumπ1_i[I] + sumπ2_i[I]
	end
end
function optimize_γi_perp!(model::MVD, i::Int64)
	@inbounds for I in eachindex(model.γ[i])
		model.γ[i][I] = model.α[I]  + model._sumπ1_i[I] + model._sumπ2_i[I]
	end
end
...
function update_local!(model::MVD,settings::Settings, mb::Vector{Int64})
	init_γs!(model, mb)
	...
	for d in 1:length(mb)
		...
		sstat_dw = [(expelogΘd .* expelogϕ1[:,v]) .+ 1e-100 for v in ids1]
		sstat_dw ./= sum.(sstat_dw)
		sstat_w = collect(hcat(sum.(sstat_dw .* cts1, dims = 2)...))
		model._sumπ1_mb[:,ids1] .+= sstat_w

		sstat_dw = [(expelogΘd .* collect(expelogϕ2[:,v]')) .+ 1e-100 for v in ids2]
		sstat_dw ./= sum.(sstat_dw)
...
function update_global!(model::MVD, ρ::Float64, count_params::TrainCounts, mb,len_mb2)
	copyto!(model._λ1_old,  model._λ1)
	optimize_λ!(model._λ1, length(mb), model._η1, model._sumπ1_mb, count_params._D1)
	model._λ1 .= (1.0-ρ).*model._λ1_old .+ ρ.*model._λ1
	dir_expectationByRow!(model._elogϕ1,model._λ1)
	copyto!(model._λ2_old,model._λ2)
	optimize_λ!(model._λ2,len_mb2, model._η2, model._sumπ2_mb,count_params._D2)
...
function update_α_newton_mb!(model::MVD,ρ, count_params::TrainCounts,mb::Vector{Int64}, h_map::Vector{Bool}, settings::Settings)
